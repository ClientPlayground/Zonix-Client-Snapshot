...
class GuiSlotWaypoints extends GuiSlot {
   ...
   public GuiSlotWaypoints(GuiWaypoints par1GuiWaypoints) {
      ...
      Iterator var2 = this.parentGui.waypointManager.getWaypoints().iterator();
   ...
   protected void sortBy(int sortKey, boolean ascending) {
      ...
      if (sortKey == 1) {
         ...
         Collections.sort(this.waypoints, new Comparator() {
            public int compare(Waypoint waypoint1, Waypoint waypoint2) {
               return Double.compare((double)masterWaypointsList.indexOf(waypoint1), (double)masterWaypointsList.indexOf(waypoint2)) * order;
            }
         });
      } else {
         this.parentGui.options.getClass();
