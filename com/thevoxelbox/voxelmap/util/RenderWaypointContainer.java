...
public class RenderWaypointContainer extends Render {
   ...
   public void doRenderWaypoints(EntityWaypointContainer par1EntityWaypointContainer, double baseX, double baseY, double baseZ, float par8, float par9) {
      Iterator var10 = par1EntityWaypointContainer.wayPts.iterator();

      while(var10.hasNext()) {
         Waypoint pt = (Waypoint)var10.next();
         if (pt.isActive()) {
            int x = pt.getX();
            int z = pt.getZ();
            int y = pt.getY();
            ...
            if (this.options.showBeacons && par1EntityWaypointContainer.worldObj.getChunkFromBlockCoords(x, z).isChunkLoaded) {
               ...
               this.renderBeam(pt, (double)x - RenderManager.renderPosX, bottomOfWorld, (double)z - RenderManager.renderPosZ, 64.0F, distance);
            }

            if (this.options.showWaypoints && !this.options.game.gameSettings.hideGUI) {
               String label = pt.name;
               this.renderLabel(pt, label, baseX + (double)x, baseY + (double)y + 1.0D, baseZ + (double)z, 64, distance);
            }
         }
      }
   ...
   public void renderBeam(Waypoint par1EntityWaypoint, double baseX, double baseY, double baseZ, float par8, double distance) {
      ...
      float alphaMultiplier = distance < 30.0D ? (float)distance / 30.0F : 1.0F;
   ...
   protected void renderLabel(Waypoint par1EntityWaypoint, String par2Str, double par3, double par5, double par7, int par9, double distance) {
      ...
      if (distance <= (double)this.options.maxWaypointDisplayDistance || this.options.maxWaypointDisplayDistance < 0) {
         ...
         double maxDistance = (double)(this.options.game.gameSettings.getOptionFloatValue(GameSettings.Options.RENDER_DISTANCE) * 16.0F) * 0.75D;
